  ...
  include AdsForPages
   ...
   def apply_job
    ...
    def all_activity
         ...
         @featured_job1 = Job.where( "is_featured = TRUE AND visibility = 0 AND status = 1 AND is_approved = TRUE AND show_on_cgmeetup = TRUE AND (publish = 1 OR (publish = 0 AND to_timestamp(schedule_time, 'YYYY-MM-DD hh24:mi')::timestamp without time zone <= CURRENT_TIMESTAMP::timestamp without time zone))").order('id DESC').limit(1)
        @non_featured_job = Job.where( "is_featured = FALSE AND visibility = 0 AND status = 1 AND is_approved = TRUE AND show_on_cgmeetup = TRUE AND (publish = 1 OR (publish = 0 AND to_timestamp(schedule_time, 'YYYY-MM-DD hh24:mi')::timestamp without time zone <= CURRENT_TIMESTAMP::timestamp without time zone))").order('id DESC').limit(6)
