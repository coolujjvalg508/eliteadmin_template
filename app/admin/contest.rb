ActiveAdmin.register Contest do
    ...
    #menu label: 'Projects', parent: 'Contest',priority: 2 
   actions :all, except: [:new]
	config.filters = false


    permit_params :is_spam, :winner_type, :is_trash, :title,:user_id,:is_admin,:paramlink, {:challenge => []}, {:skill => []}, {:team_member => []},:show_on_cgmeetup,:show_on_website, :schedule_time, :description, :post_type_category_id, 
	 ...
	 controller do 
		...
		def action_methods
		  super                                  
				...
				disallowed = []
				disallowed << 'index' if (!usergroup.has_permission('contest_read') && !usergroup.has_permission('contest_write') && !usergroup.has_permission('contest_delete'))
				disallowed << 'delete' unless (usergroup.has_permission('contest_delete'))
				disallowed << 'create' unless (usergroup.has_permission('contest_write'))
				disallowed << 'new' unless (usergroup.has_permission('contest_write'))
				disallowed << 'edit' unless (usergroup.has_permission('contest_write'))
				disallowed << 'destroy' unless (usergroup.has_permission('contest_delete'))
	...
	form multipart: true do |f|
		...
		#  f.input :schedule_time, as: :date_time_picker
		  ...
		  f.input :winner_type, as: :select, collection: [['Winner 1',1],['Winner 2',2],['Winner 3',3],['Honourable Mentions',4]], :input_html => { :class => "chosen-input" }, include_blank: false, multiple: false		    
  ...
  controller do
	...
	def create
			params[:contest][:user_id] = current_admin_user.id.to_s
			params[:contest][:is_admin] = 'Y'
			if (params[:contest].present? && params[:contest][:images_attributes].present?)
					params[:contest][:images_attributes].each do |index,img|
						  unless params[:contest][:images_attributes][index][:image].present?
							params[:contest][:images_attributes][index][:image] = params[:contest][:images_attributes][index][:image_cache]
							params[:contest][:images_attributes][index][:caption_image] = params[:contest][:images_attributes][index][:caption_image]
						  end
					end
				super
			
			elsif (params[:contest].present? && params[:contest][:videos_attributes].present?)
					params[:contest][:videos_attributes].each do |index,img|
						  unless params[:contest][:videos_attributes][index][:video].present?
							params[:contest][:videos_attributes][index][:video] = params[:contest][:videos_attributes][index][:video_cache]
							params[:contest][:videos_attributes][index][:caption_video] = params[:contest][:videos_attributes][index][:caption_video]
						  end
					end
				super
				
				
			elsif (params[:contest].present? && params[:contest][:upload_videos_attributes].present?)
					params[:contest][:upload_videos_attributes].each do |index,img|
						  unless params[:contest][:upload_videos_attributes][index][:uploadvideo].present?
							params[:contest][:upload_videos_attributes][index][:uploadvideo] = params[:contest][:upload_videos_attributes][index][:uploadvideo_cache]
							params[:contest][:upload_videos_attributes][index][:caption_upload_video] = params[:contest][:upload_videos_attributes][index][:caption_upload_video]
						  end
					end
				super	
				
				
			elsif (params[:contest].present? && params[:contest][:sketchfebs_attributes].present?)
					params[:contest][:sketchfebs_attributes].each do |index,img|
						  unless params[:contest][:sketchfebs_attributes][index][:sketchfeb].present?
							params[:contest][:sketchfebs_attributes][index][:sketchfeb] = params[:contest][:sketchfebs_attributes][index][:sketchfeb_cache]
							params[:contest][:sketchfebs_attributes][index][:caption_sketchfeb] = params[:contest][:sketchfebs_attributes][index][:caption_sketchfeb]
						  end
					end
				super
				
			elsif (params[:contest].present? && params[:contest][:marmosets_attributes].present?)
					params[:contest][:marmosets_attributes].each do |index,img|
						  unless params[:contest][:marmosets_attributes][index][:marmoset].present?
							params[:contest][:marmosets_attributes][index][:marmoset] = params[:contest][:marmosets_attributes][index][:marmoset_cache]
							params[:contest][:marmosets_attributes][index][:marmoset] = params[:contest][:marmosets_attributes][index][:caption_marmoset]
						  end
					end
				super
		  else
		...
		def update
			...
			super
				...
				super
					...
					params[:contest][:upload_videos_attributes][index][:uploadvideo]  = params[:contest][:upload_videos_attributes][index][:uploadvideo]	  
					end
				super	
...
=end
 ...
 /
					...
					contest.videos.each do |img|
						span do
								if(img.video.include?('youtube'))	
										...
										raw('<iframe title="Contest Video" width="300" height="200" src="https://www.youtube.com/embed/' + youtube_id + '" frameborder="0" allowfullscreen></iframe>')
		  ...
		  row 'Uploaded Videos' do
			ul class: "image-blk" do
				if contest.upload_videos.present?
				  contest.upload_videos.each do |img|
					span do
						raw('<iframe title="Contest Video" width="300" height="200" src="'+img.uploadvideo.url+'" frameborder="0" allowfullscreen></iframe>')
	...
	#	Contest.where(id: ids).update_all(is_spam: inputs[:is_spam])
	...
	#	Contest.where(id: ids).update_all(status: inputs[:status])
