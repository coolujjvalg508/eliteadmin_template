...
# end
  ...
  index :download_links => ['csv'] do
    ...
    column :topic_for do |cat|
		(cat.topic_for == 1) ? 'News' : (cat.topic_for == 2) ? 'Download' : 'Tutorial'
      end
 ...
 form multipart: true do |f|
      ...
      f.input :topic_for, :as => :select, collection: [['News',1], ['Tutorial', 0],['Download', 2]], include_blank: 'Select'
      f.input :image
      f.input :name
      f.input :description
      f.input :slug
  ...
  filter :name
  filter :created_at
  filter :topic_for, as: :select, collection: [['News',1], ['Tutorial', 0],['Download', 2]], label: 'Select Topic for'
